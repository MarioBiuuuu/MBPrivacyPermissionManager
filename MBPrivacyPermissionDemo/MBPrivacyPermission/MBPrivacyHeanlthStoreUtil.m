...
static MBPrivacyHeanlthStoreUtil *_instance = nil;

@implementation MBPrivacyHeanlthStoreUtil
    ...
    if ([[[UIDevice currentDevice] systemVersion] doubleValue] >= 8.0) {
        if (![HKHealthStore isHealthDataAvailable]) {
            ...
            [store requestAuthorizationToShareTypes:writeObjectTypes readTypes:readObjectTypes completion:^(BOOL success, NSError * _Nullable error) {
                if (success == YES) {
                    block(MBHealthPermissionResponseSuccess);
                }else{
                    block(MBHealthPermissionResponseError);
                }
            }];
        }
    }else{
        DEFLog(@"HealthKit暂不支持iOS8以下系统,请更新你的系统。");
    }
}
